    ...
    return (
        <>
            <div className='container' style={{ color: props.mode === 'dark' ? 'white' : 'black' }}>
                <h1>{props.heading}</h1>
                <div className="mb-3">
                    <textarea className="form-control" id="myBox" style={{ color: props.mode === 'dark' ? 'white' : 'black', backgroundColor: props.mode === 'dark' ? '#343541' : 'white' }} onChange={handleOnChange} value={text} rows="8"></textarea>
                </div>
                <button className={props.mode === 'dark' ? "btn btn-outline-info my-1 mx-1" : "btn btn-info my-1 mx-1"} onClick={handleUpClick}>Convert to Uppercase</button>
                ...
                <button className={props.mode === 'dark' ? "btn btn-outline-danger my-1 mx-1" : "btn btn-danger my-1 mx-1"} onClick={handleClearClick}>Clear text</button>
            </div>
            <div className="container my-3" style={{ color: props.mode === 'dark' ? 'white' : 'black' }}>
                <h1>Your text summary</h1>
                <p>{wordCount} words, {text.length} characters</p>
                <p>{wordCount === 0 ? 0 : 0.008 * text.split(/\s+/).length} Minutes read</p>
                <h2>Preview</h2>
